...
class UCF101Dataset(Dataset):
    def __init__(
        ...
        with self.videos_ds.opener(self.videos_ds.shard_paths[0]) as f:
            ...
            self.num_test_vids = (
                f["num_test"][()] // 10
            )  # https://arxiv.org/pdf/1511.05440.pdf takes every 10th test video
    ...
    def __getitem__(self, index, time_idx=0):
        ...
        if not self.train:
            video_index = (
                video_index * 10 + self.num_train_vids
            )  # https://arxiv.org/pdf/1511.05440.pdf takes every 10th test video
        shard_idx, idx_in_shard = self.videos_ds.get_indices(video_index)

        # random crop
        crop_c = (
            np.random.randint(int(self.image_size / 240 * 320) - self.image_size)
            if self.train
            else int((self.image_size / 240 * 320 - self.image_size) / 2)
        )
        ...
        with self.videos_ds.opener(self.videos_ds.shard_paths[shard_idx]) as f:
            ...
            for i in range(time_idx, min(time_idx + self.frames_per_sample, video_len)):
                ...
                arr = transforms.RandomHorizontalFlip(flip_p)(
                    transforms.ToTensor()(img[:, crop_c : crop_c + self.image_size])
                )
