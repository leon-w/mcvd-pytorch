...
def parse_args_and_config():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Batch size in InceptionNetV3 for FID calc",
    )
    parser.add_argument(
        "--no_pr",
        action="store_true",
        help="No PR calc, only FID calc. Generally unnecessary.",
    )
    parser.add_argument(
        "--fid_num_samples",
        type=int,
        default=None,
        help="# of samples for FID, to override config.fast_fid.num_samples, when using sample/test/fast_fid",
    )
    parser.add_argument(
        "--pr_nn_k",
        type=int,
        default=None,
        help="# of nearest neighbours for Precision/Recall, to override config.fast_fid.pr_nn_k, when using sample/test/fast_fid",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        choices=[True, False],
        help="Whether to save ONLY final image or all sampling steps, when using sample/test/fast_fid",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Model checkpoint # to load until, when using test/fast_fid",
    )
    parser.add_argument(
        "--freq",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="videos",
        help="The folder name of video samples",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="For KTH, can start at Kth frame in test and ignore the rest",
    )
    ...
    if not args.test and not args.sample and not args.video_gen and not args.fast_fid:
        ...
        if args.sample:
            ...
            if new_config.sampling.final_only:
                ...
                args.image_folder = os.path.join(
                    args.exp,
                    f"image_samples_{new_config.sampling.ckpt_id}",
                    args.image_folder,
                )
...
def copy_scripts(src, dst):
    ...
    for d in glob.glob(os.path.join(src, "*/")):
        if (
            ...
            and os.path.basename(os.path.dirname(d)) != "assets"
        ):
