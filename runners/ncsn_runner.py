...
class NCSNRunner:
    ...
    def train(self):
        ...
        if self.config.data.dataset.upper() == "FFHQ":
            ...
            test_loader = FFHQ_TFRecordsDataLoader(
                [self.args.data_path],
                self.config.training.batch_size,
                self.config.data.image_size,
            )
        ...
        if self.args.resume_training:
            ...
            logging.info(
                f"Resuming training from checkpoint.pt in {self.args.log_path} at epoch {start_epoch}, step {step}."
            )
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y) in enumerate(dataloader):
                ...
                if step == 1 or step % self.config.training.val_freq == 0:
                    ...
                    test_X, test_cond, test_cond_mask = conditioning_fn(
                        self.config,
                        test_X,
                        num_frames_pred=self.config.data.num_frames,
                        prob_mask_cond=getattr(self.config.data, "prob_mask_cond", 0.0),
                ...
                if (
                    ...
                    if (
                        ...
                        if self.condp == 0.0 and self.futrf == 0:  # (1) Prediction
                            ...
                            (
                                self.best_mse_interp,
                                self.best_psnr_interp,
                                self.best_ssim_interp,
                                self.best_lpips_interp,
                                self.best_fvd_interp,
                                self.calc_fvd_interp,
                            ) = (
                                self.best_mse,
                                self.best_psnr,
                                self.best_ssim,
                        ...
                        format_p = lambda dd: ", ".join(
                            [
                                f"{k}:{v:.4f}"
                                if k != "ckpt" and k != "preds_per_test"
                                else f"{k}:{v:7d}"
                                if k == "ckpt"
                                else f"{k}:{v:3d}"
                                for k, v in dd.items()
                    ...
                    if conditional:
                        ...
                        if self.config.data.channels == 1:
                            ...
                            padding_red, padding_green = torch.ones(
                                len(reali),
                                self.config.data.channels,
                                self.config.data.image_size,
                                4,
    ...
    def plot_graphs(self):
        ...
        # Time elapsed
        plt.plot(
            self.losses_train.steps[: len(self.time_elapsed.vals)],
            self.time_elapsed.vals,
        )
        plt.xlabel("Steps")
        plt.grid(True)
        plt.grid(visible=True, which="minor", axis="y", linestyle="--")
    ...
    def plot_video_graphs_single(
        ...
        if best_mse["ckpt"] > -1:
            plt.scatter(
                best_mse["ckpt"],
                mses.vals[mses.steps.index(best_mse["ckpt"])],
                color="k",
            )
            plt.text(
                best_mse["ckpt"],
                mses.vals[mses.steps.index(best_mse["ckpt"])],
                f"{mses.vals[mses.steps.index(best_mse['ckpt'])]:.04f}\n{best_mse['ckpt']}",
                c="r",
            )
        plt.xlabel("Steps")
        plt.ylabel("MSE")
        ...
        if best_psnr["ckpt"] > -1:
            plt.scatter(
                best_psnr["ckpt"],
                psnrs.vals[mses.steps.index(best_psnr["ckpt"])],
                color="k",
            )
            plt.text(
                best_psnr["ckpt"],
                psnrs.vals[mses.steps.index(best_psnr["ckpt"])],
                f"{psnrs.vals[mses.steps.index(best_psnr['ckpt'])]:.04f}\n{best_psnr['ckpt']}",
                c="r",
            )
        plt.xlabel("Steps")
        plt.ylabel("PSNR")
        ...
        if best_ssim["ckpt"] > -1:
            plt.scatter(
                best_ssim["ckpt"],
                ssims.vals[mses.steps.index(best_ssim["ckpt"])],
                color="k",
            )
            plt.text(
                best_ssim["ckpt"],
                ssims.vals[mses.steps.index(best_ssim["ckpt"])],
                f"{ssims.vals[mses.steps.index(best_ssim['ckpt'])]:.04f}\n{best_ssim['ckpt']}",
                c="r",
            )
        plt.xlabel("Steps")
        plt.ylabel("SSIM")
        ...
        if best_lpips["ckpt"] > -1:
            plt.scatter(
                best_lpips["ckpt"],
                lpipss.vals[mses.steps.index(best_lpips["ckpt"])],
                color="k",
            )
            plt.text(
                best_lpips["ckpt"],
                lpipss.vals[mses.steps.index(best_lpips["ckpt"])],
                f"{lpipss.vals[mses.steps.index(best_lpips['ckpt'])]:.04f}\n{best_lpips['ckpt']}",
                c="r",
            )
        plt.xlabel("Steps")
        plt.ylabel("LPIPS")
        ...
        if calc_fvd:
            ...
            if best_fvd["ckpt"] > -1:
                plt.scatter(
                    best_fvd["ckpt"],
                    fvds.vals[mses.steps.index(best_fvd["ckpt"])],
                    color="k",
                )
                plt.text(
                    best_fvd["ckpt"],
                    fvds.vals[mses.steps.index(best_fvd["ckpt"])],
                    f"{fvds.vals[mses.steps.index(best_fvd['ckpt'])]:.04f}\n{best_fvd['ckpt']}",
                    c="r",
                )
            plt.xlabel("Steps")
            plt.ylabel("FVD")
    ...
    def plot_video_graphs(self):
        ...
        if self.mode_gen is not None and self.calc_fvd3:
            ...
            if self.best_fvd3["ckpt"] > -1:
                plt.scatter(
                    self.best_fvd3["ckpt"],
                    self.fvds3.vals[self.fvds3.steps.index(self.best_fvd3["ckpt"])],
                    color="k",
                )
                plt.text(
                    self.best_fvd3["ckpt"],
                    self.fvds3.vals[self.fvds3.steps.index(self.best_fvd3["ckpt"])],
                    f"{self.fvds3.vals[self.fvds3.steps.index(self.best_fvd3['ckpt'])]:.04f}\n{self.best_fvd3['ckpt']}",
                    c="r",
                )
            plt.xlabel("Steps")
            plt.ylabel("FVD")
    ...
    def sample(self):
        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                refer_images, cond, _ = conditioning_fn(
                    self.config,
                    refer_images,
                    num_frames_pred=self.config.data.num_frames,
                    conditional=conditional,
                ...
                torch.save(
                    refer_images[:width, ...],
                    os.path.join(self.args.image_folder, "refer_image.pt"),
                )
                ...
                if conditional:
                    ...
                    real = stretch_image(
                        inverse_data_transform(self.config, real),
                        self.config.data.channels,
                        self.config.data.image_size,
                    )
                    if future > 0:
                        ...
                        futr = stretch_image(
                            inverse_data_transform(self.config, futr),
                            self.config.data.channels,
                            self.config.data.image_size,
                        )
                    cond = stretch_image(
                        inverse_data_transform(self.config, cond),
                        self.config.data.channels,
                        self.config.data.image_size,
                    )
                    padding = 0.5 * torch.ones(
                    ...
                    image_grid = make_grid(
                        torch.cat(
                            [cond, padding, real, padding, sample]
                            if future == 0
                            else [cond, padding, real, padding, sample, futr],
                            dim=-1,
                        ),
                        nrow=nrow,
                        padding=6,
                    ...
                    save_image(
                        ...
                        os.path.join(
                            self.args.image_folder,
                            "image_full_grid_{}.png".format(ckpt),
                        ),
                    )
                    torch.save(
                        sample,
            ...
            if conditional:
                if self.config.data.dataset.upper() == "FFHQ":
                    dataloader_cond = FFHQ_TFRecordsDataLoader(
                        [self.args.data_path],
                        self.config.sampling.batch_size,
                        self.config.data.image_size,
                    )
            ...
            for i in tqdm(range(n_rounds), desc="Generating samples for FID"):
                ...
                final_samples = all_samples[-1].reshape(
                    all_samples[-1].shape[0],
                    self.config.data.channels * self.config.data.num_frames,
                    self.config.data.image_size,
                    self.config.data.image_size,
            ...
            # Save samples
            # Stretch out multiple frames horizontally
            gen_samples_to_save = stretch_image(
                gen_samples[: self.config.sampling.batch_size],
                self.config.data.channels,
                self.config.data.image_size,
            )
            nrow = ceil(
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        if calc_fvd:
            ...
            self.calc_fvd1, self.calc_fvd2, self.calc_fvd3 = (
                calc_fvd1,
                calc_fvd2,
                calc_fvd3,
            )
        ...
        assert (
            conditional
        ), f"Video generating model has to be conditional! num_frames_cond has to be > 0! Given {self.config.data.num_frames_cond}"
        cond = None
        prob_mask_cond = getattr(self.config.data, "prob_mask_cond", 0.0)
        ...
        model_lpips = eval_models.PerceptualLoss(
            model="net-lin", net="alex", device=self.config.device
        )  # already in test mode and dataparallel
        # model_lpips = torch.nn.DataParallel(model_lpips)
        # model_lpips.eval()
        # Sampler
        sampler = self.get_sampler()

        for i, (real_, _) in tqdm(
            ...
            # Video Interpolation
                ...
                num_frames_pred = self.config.data.num_frames
                logging.info(
                    f"INTERPOLATING {num_frames_pred} frames, using a {self.config.data.num_frames} frame model conditioned on {self.config.data.num_frames_cond} cond + {future} future frames, subsample={getattr(self.config.sampling, 'subsample', None)}, preds_per_test={preds_per_test}"
                )

            real, cond, cond_mask = conditioning_fn(
                self.config,
                real_,
                num_frames_pred=num_frames_pred,
                prob_mask_cond=0.0,
                prob_mask_future=0.0,
                conditional=conditional,
            )
            ...
            if self.config.sampling.data_init:
                try:
                    ...
                    if self.config.data.dataset.upper() == "FFHQ":
                        dataloader2 = FFHQ_TFRecordsDataLoader(
                            [self.args.data_path],
                            self.config.sampling.batch_size,
                            self.config.data.image_size,
                        )
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                if cond is None:  # first frames are the cond
                    ...
                    cond = torch.cat(
                        [
                            cond[:, self.config.data.channels :],
                ...
                if self.config.sampling.data_init:
                    if getattr(self.config.sampling, "one_frame_at_a_time", False):
                        real_init1 = real_init[
                            ...
                            * (i_frame + 1 + self.config.data.num_frames)
                        ]
                    else:
                        real_init1 = real_init[
                            (i_frame + 1)
                            * self.config.data.channels
                            * self.config.data.num_frames : (i_frame + 2)
                            * self.config.data.channels
                            * self.config.data.num_frames
                        ]
                    if self.version == "SMLD":
            ...
            if real.shape[1] < pred.shape[1]:  # We cannot calculate MSE, PSNR, SSIM
                print(
                    "-------- Warning: Cannot calculate metrics because predicting beyond the training data range --------"
                )
            ...
            if future > 0 and prob_mask_future > 0.0 and not self.prob_mask_sync:
                ...
                real2, cond, cond_mask = conditioning_fn(
                    self.config,
                    real_,
                    num_frames_pred=num_frames_pred,
                    prob_mask_cond=0.0,
                    prob_mask_future=1.0,
                    conditional=conditional,
                )
                    ...
                    if cond is None:  # first frames are the cond
                        ...
                        cond = torch.cat(
                            [
                                ...
                                gen_samples[:, : self.config.data.channels],
                                cond[
                                    :, -self.config.data.channels * future :
                                ],  # future frames are always there, but always 0
                            ],
                            dim=1,
                        )
                    else:
                        cond = torch.cat(
                            [
                                cond[
                                    :,
                                    self.config.data.channels
                                    * self.config.data.num_frames : -self.config.data.channels
                                    * future,
                                ],
                                gen_samples[
                                    :,
            ...
            if calc_fvd1 or calc_fvd2 or calc_fvd3:
                ...
                if calc_fvd3:
                    ...
                    # We mask cond
                    _, cond_fvd, cond_mask_fvd = conditioning_fn(
                        self.config,
                        real_,
                        num_frames_pred=num_frames_pred,
                        prob_mask_cond=1.0,
            ...
            if i == 0 or preds_per_test == 1:  # Save first mini-batch or save them all
                ...
                if (
                    future > 0
                ):  # if conditional, we show conditional frames, and real&pred, and future frames side-by-side
                ...
                def save_pred(pred, real):
                    ...
                    if train:
                        save_image(
                            ...
                            os.path.join(
                                self.args.log_sample_path,
                                f"videos_stretch_pred_{ckpt}_{i}.png",
                            ),
                        )
                    else:
                        save_image(
                ...
                def save_gen(pred):
                    ...
                    if train:
                        save_image(
                            ...
                            os.path.join(
                                self.args.log_sample_path,
                                f"videos_stretch_gen_{ckpt}_{i}.png",
                            ),
                        )
                    else:
                        save_image(
        ...
        vid_metrics = {
            "ckpt": ckpt,
            "preds_per_test": preds_per_test,
            "mse": avg_mse,
            "mse_std": std_mse,
            "mse_conf95": conf95_mse,
            "psnr": avg_psnr,
            "psnr_std": std_psnr,
            "psnr_conf95": conf95_psnr,
            "ssim": avg_ssim,
            "ssim_std": std_ssim,
            "ssim_conf95": conf95_ssim,
            "lpips": avg_lpips,
            "lpips_std": std_lpips,
            "lpips_conf95": conf95_lpips,
        }
        ...
        if calc_fvd1 or calc_fvd2 or calc_fvd3:
            ...
            if calc_fvd1:
                ...
                vid_metrics.update(
                    {
                        "fvd": avg_fvd,
                        "fvd_traj_mean": fvd_traj_mean,
                        "fvd_traj_std": fvd_traj_std,
                        "fvd_traj_conf95": fvd_traj_conf95,
                    }
                )
        ...
        if second_calc:
            ...
            vid_metrics.update(
                {
                    "mse2": avg_mse2,
                    "mse2_std": std_mse2,
                    "mse2_conf95": conf95_mse2,
                    "psnr2": avg_psnr2,
                    "psnr2_std": std_psnr2,
                    "psnr2_conf95": conf95_psnr2,
                    "ssim2": avg_ssim2,
                    "ssim2_std": std_ssim2,
                    "ssim2_conf95": conf95_ssim2,
                    "lpips2": avg_lpips2,
                    "lpips2_std": std_lpips2,
                    "lpips2_conf95": conf95_lpips2,
                }
            )
            ...
            if calc_fvd2:
                ...
                avg_fvd2, fvd2_traj_mean, fvd2_traj_std, fvd2_traj_conf95 = fvd_stuff(
                    fake_embeddings2, real_embeddings2
                )
                vid_metrics.update(
                    {
                        "fvd2": avg_fvd2,
                        "fvd2_traj_mean": fvd2_traj_mean,
                        "fvd2_traj_std": fvd2_traj_std,
                        "fvd2_traj_conf95": fvd2_traj_conf95,
                    }
                )
        ...
        if calc_fvd3:
            ...
            avg_fvd3, fvd3_traj_mean, fvd3_traj_std, fvd3_traj_conf95 = fvd_stuff(
                fake_embeddings_uncond, real_embeddings_uncond
            )
            vid_metrics.update(
                {
                    "fvd3": avg_fvd3,
                    "fvd3_traj_mean": fvd3_traj_mean,
                    "fvd3_traj_std": fvd3_traj_std,
                    "fvd3_traj_conf95": fvd3_traj_conf95,
                }
            )

        if not train and (calc_fvd1 or calc_fvd2 or calc_fvd3):
            np.savez(
                os.path.join(self.args.video_folder, f"video_embeddings_{ckpt}.npz"),
        ...
        format_p = lambda dd: ", ".join(
            [
                f"{k}:{v:.4f}"
                if k != "ckpt" and k != "preds_per_test" and k != "time"
                else f"{k}:{v:7d}"
                if k == "ckpt"
                else f"{k}:{v:3d}"
                if k == "preds_per_test"
        ...
        if train:
            ...
            if self.condp == 0.0 and self.futrf == 0:  # (1) Prediction
                ...
                (
                    ...
                    vid_metrics["pred_ssim"],
                    vid_metrics["pred_lpips"],
                ) = (
                    vid_metrics["mse"],
                    vid_metrics["psnr"],
                    vid_metrics["ssim"],
                ...
                (
                    vid_metrics["pred_mse_std"],
                    vid_metrics["pred_psnr_std"],
                    vid_metrics["pred_ssim_std"],
                    vid_metrics["pred_lpips_std"],
                ) = (
                    vid_metrics["mse_std"],
                    vid_metrics["psnr_std"],
                    vid_metrics["ssim_std"],
                    vid_metrics["lpips_std"],
                )
                (
                    vid_metrics["pred_mse_conf95"],
                    vid_metrics["pred_psnr_conf95"],
                    vid_metrics["pred_ssim_conf95"],
                    vid_metrics["pred_lpips_conf95"],
                ) = (
                    vid_metrics["mse_conf95"],
                    vid_metrics["psnr_conf95"],
                    vid_metrics["ssim_conf95"],
                    vid_metrics["lpips_conf95"],
                )
                if calc_fvd1:
                    (
                        vid_metrics["pred_fvd"],
                        vid_metrics["pred_fvd_traj_mean"],
                        vid_metrics["pred_fvd_traj_std"],
                        vid_metrics["pred_fvd_traj_conf95"],
                    ) = (
                        vid_metrics["fvd"],
                        vid_metrics["fvd_traj_mean"],
                        vid_metrics["fvd_traj_std"],
                ...
                (
                    ...
                    vid_metrics["interp_ssim"],
                    vid_metrics["interp_lpips"],
                ) = (
                    vid_metrics["mse"],
                    vid_metrics["psnr"],
                    vid_metrics["ssim"],
                ...
                (
                    vid_metrics["interp_mse_std"],
                    vid_metrics["interp_psnr_std"],
                    vid_metrics["interp_ssim_std"],
                    vid_metrics["interp_lpips_std"],
                ) = (
                    vid_metrics["mse_std"],
                    vid_metrics["psnr_std"],
                    vid_metrics["ssim_std"],
                ...
                (
                    vid_metrics["interp_mse_conf95"],
                    vid_metrics["interp_psnr_conf95"],
                    vid_metrics["interp_ssim_conf95"],
                    vid_metrics["interp_lpips_conf95"],
                ) = (
                    vid_metrics["mse_conf95"],
                    vid_metrics["psnr_conf95"],
                    vid_metrics["ssim_conf95"],
                ...
                if calc_fvd1:
                    (
                        vid_metrics["interp_fvd"],
                        vid_metrics["interp_fvd_traj_mean"],
                        vid_metrics["interp_fvd_traj_std"],
                        vid_metrics["interp_fvd_traj_conf95"],
                    ) = (
                        vid_metrics["fvd"],
                        vid_metrics["fvd_traj_mean"],
                        vid_metrics["fvd_traj_std"],
                ...
                if second_calc:
                    ...
                    (
                        ...
                        vid_metrics["mse2_std"],
                        vid_metrics["psnr2_std"],
                        vid_metrics["ssim2_std"],
                        vid_metrics["lpips2_std"],
                    )
                    (
                        vid_metrics["pred_mse_conf95"],
                        vid_metrics["pred_psnr_conf95"],
                        ...
                        vid_metrics["mse2_conf95"],
                        vid_metrics["psnr2_conf95"],
                        vid_metrics["ssim2_conf95"],
                        vid_metrics["lpips2_conf95"],
                    )
                if calc_fvd2:
                    (
                        vid_metrics["pred_fvd"],
                        ...
                        vid_metrics["fvd2"],
                        vid_metrics["fvd2_traj_mean"],
                        vid_metrics["fvd2_traj_std"],
                        vid_metrics["fvd2_traj_conf95"],
                    )
                ...
                if calc_fvd3:
                    (
                        vid_metrics["gen_fvd"],
                        vid_metrics["gen_fvd_traj_mean"],
                        vid_metrics["gen_fvd_traj_std"],
                        vid_metrics["gen_fvd_traj_conf95"],
                    ) = (
                        vid_metrics["fvd3"],
                        vid_metrics["fvd3_traj_mean"],
                        vid_metrics["fvd3_traj_std"],
                        vid_metrics["fvd3_traj_conf95"],
                    )
    ...
    def test(self):
        ...
        if self.config.data.dataset.upper() == "FFHQ":
            test_dataloader = FFHQ_TFRecordsDataLoader(
                [self.args.data_path],
        ...
        for ckpt in tqdm(
            range(
                self.config.test.begin_ckpt,
                self.config.test.end_ckpt + 1,
                getattr(self.config.test, "freq", 5000),
            ),
    ...
    def fast_fid(self):
        ...
        for ckpt in tqdm(
            range(
                self.config.fast_fid.begin_ckpt,
                self.config.fast_fid.end_ckpt + 1,
                getattr(self.config.fast_fid, "freq", 5000),
            ),
            desc="Ckpt",
        ):
            ...
            # Generate samples
                ...
                for i in tqdm(range(num_iters), desc="samples"):
                    ...
                    if self.version == "SMLD":
                        ...
                        if getattr(self.config.model, "gamma", False):
                            ...
                            z = (
                                Gamma(
                                    torch.full(real.shape, used_k),
                                    torch.full(real.shape, 1 / used_theta),
                                )
                                .sample()
                                .to(self.config.device)
                            )
                ...
                image_grid = make_grid(
                    gen_samples[: self.config.data.num_frames * 100],
                    nrow=nrow,
                    pad_value=0.5,
                )
                save_image(
            ...
            # FID, precision, recall
                ...
                self.write_to_yaml(
                    os.path.join(self.args.image_folder, f"precisions_k{k}.yml"),
                    precisions,
                )
                self.write_to_pickle(
